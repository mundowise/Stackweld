...
StackPilot is a local-first developer tool that generates complete, working project structures from a stack definition. It eliminates manual project setup by delegating to official scaffold tools (create-next-app, django-admin, etc.) and layering additional configuration on top.

**Core principle:** StackPilot never replaces official CLI tools — it orchestrates them.
...
Database location: `~/.stackpilot/stackpilot.db`
...
Custom templates follow the same `Template` interface and are stored locally in `~/.stackpilot/templates/`.
...
**Context:** StackPilot has five distinct packages with a clear dependency graph. A monorepo avoids duplication of types and utilities across packages. The toolchain choice affects developer experience significantly.
...
**Context:** StackPilot is a local-first tool. Stack data lives on the developer's machine. The data model is simple (five tables, no complex queries). Writes happen during scaffold (infrequent). Reads happen during validation and list commands (frequent, small datasets).
...
**Context:** StackPilot could generate all project files from templates. This would give full control but requires maintaining templates that track every framework's evolving conventions.

**Decision:** For technologies with an `officialScaffold` command, StackPilot runs that command and layers configuration on top. StackPilot never replaces the official generator.
...
- Positive: Framework updates are absorbed automatically — StackPilot does not need to be updated when Next.js changes its default structure.
...
**Context:** A visual interface for StackPilot requires a cross-platform desktop framework. The main candidates are Electron and Tauri.

**Decision:** Tauri 2 with React 19 frontend.

**Alternatives considered:**
1. Electron — rejected because Electron bundles Chromium (~150MB) and a Node.js runtime. A StackPilot desktop binary would be 200MB+. Tauri uses the system WebView and a Rust backend, producing binaries under 10MB.
...
User preferences are stored in `~/.stackpilot/config.json`. Database is at `~/.stackpilot/stackpilot.db`. Custom templates are stored in `~/.stackpilot/templates/`.
