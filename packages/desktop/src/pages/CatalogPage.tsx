...
import { useAppStore, type Technology } from "@/stores/app-store";
import { Card } from "@/components/ui/Card";
import { Badge } from "@/components/ui/Badge";
import { cn } from "@/lib/cn";

const CATEGORIES = ["all", "runtime", "frontend", "backend", "database", "orm", "auth", "styling", "service", "devops"] as const;
    ...
    <div className="space-y-6">
      ...
      {activeCategory === "all" ? (
        Object.entries(grouped).map(([category, techs]) => (
          <div key={category}>
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-3 capitalize">{category}</h2>
...
function TechCard({ tech }: { tech: Technology }) {
  ...
  return (
    <Card hoverable onClick={() => setExpanded(!expanded)} className={cn(expanded && "border-indigo-500/30")}>
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-zinc-800 text-xs space-y-2">
          {tech.requires.length > 0 && (
            <p><span className="text-zinc-500">Requires:</span> {tech.requires.join(", ")}</p>
          )}
          {tech.incompatibleWith.length > 0 && (
            <p><span className="text-red-400">Incompatible:</span> {tech.incompatibleWith.join(", ")}</p>
          )}
          {tech.suggestedWith.length > 0 && (
            <p><span className="text-green-400">Works well with:</span> {tech.suggestedWith.join(", ")}</p>
          )}
        </div>
      )}
    </Card>
